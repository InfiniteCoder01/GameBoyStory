...
//     if (other.getComponent<Mario>((uint16_t)ComponentType::Mario) && !buttonX.pressed) return;
...
//     if (joyMoved && !g_State.mario.inventory.empty()) inventoryItem = wrap(inventoryItem + joy.x + joy.y * width, g_State.mario.inventory.size());
